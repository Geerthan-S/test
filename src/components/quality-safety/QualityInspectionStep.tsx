...
export function QualityInspectionStep({
  number,
  title,
  description,
  icon: Icon,
  index,
}: QualityInspectionStepProps) {
  return (
    <motion.li
    ...
    >
      ...
      {/* Timeline dot with quality icon */}
      <motion.div
        className="delivery-step__dot flex items-center justify-center bg-[#8A3841] border-[3px] border-[#FAF8F7] rounded-full shadow-[0_0_0_1px_rgba(138,56,65,0.22)] absolute z-20 transition-all duration-300"
        style={{ width: '22px', height: '22px', left: '22px', top: '8px' }}
        aria-hidden="true"
        whileHover={{ scale: 1.25, backgroundColor: "#6D2B32" }}
